...
def _clean_file(input_path: str) -> FileResult:
    """清理单个文件并保存为 _Q 副本；失败记录原因，不抛异常。"""
    try:
...
def run_batch(target: str, on_progress=None) -> BatchResult:
    ...
    for index, input_path in enumerate(input_paths, start=1):
        ...
        results.append(_clean_file(input_path))
